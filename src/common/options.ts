...
export function getOptions(filePath: fs.PathOrFileDescriptor) {
    ...
    for (const option of optionsInStr) {
        const name = _.camelCase(option[0]);
        const params = option.slice(1).filter(op => op !== '');
        if (isOption(name)) {
            if (isTrueOption(name)) {
                resOptions = {...resOptions, [name as keyof typeof resOptions]: true };
            } else if (isFalseOption(name)) {
                resOptions = {...resOptions, [name as keyof typeof resOptions]: false };
            } else if (isStringOption(name)) {
                resOptions = {...resOptions, [name as keyof typeof resOptions]: params[0] };
